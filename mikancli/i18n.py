...
TRANSLATIONS: dict[str, dict[str, str]] = {
    "en": {
        ...
        "arg.min_episode.help": "Only accept anime-style release titles at or above this episode number, such as '- 1126'.",
        ...
        "filters.min_episode_prompt": "Enter minimum episode number, or press Enter to skip: ",
        "filters.min_episode_field": "minimum episode",
        "filters.positive_int_required": "{field} must be a positive whole number.",
        ...
        "qb.submit.rss_limit_offer": "qBittorrent is currently keeping only {current} RSS articles per feed. Minimum-episode filters may need at least {minimum}. Increase it now?",
        "qb.submit.rss_limit_skipped": "Continuing with qBittorrent's current RSS article limit of {current}.",
        "qb.submit.rss_limit_updated": "qBittorrent RSS article limit updated to {minimum}.",
        ...
        "save.content_folder_prompt": "Enter the folder and qBittorrent rule name for downloaded content (press Enter to use the default title from Mikan)",
        ...
        "display.min_episode": "Minimum episode: {value}",
        ...
        "display.items_page": "Items: {count} | Page {page}/{pages}",
    ...
    "zh-CN": {
        ...
        "arg.min_episode.help": "只接受达到此集数或更高集数的动画发布标题，例如 '- 1126'。",
        ...
        "filters.min_episode_prompt": "输入最低集数；直接按 Enter 跳过：",
        "filters.min_episode_field": "最低集数",
        "filters.positive_int_required": "{field}必须是正整数。",
        ...
        "qb.submit.rss_limit_offer": "qBittorrent 当前每个 RSS 订阅只保留 {current} 个条目。最低集数过滤可能至少需要 {minimum} 个。现在增加吗？",
        "qb.submit.rss_limit_skipped": "将继续使用 qBittorrent 当前的 RSS 条目上限：{current}。",
        "qb.submit.rss_limit_updated": "qBittorrent RSS 条目上限已更新为 {minimum}。",
        ...
        "save.content_folder_prompt": "输入下载内容的文件夹和 qBittorrent 规则名称（按 Enter 使用 Mikan 的默认标题）",
        ...
        "display.min_episode": "最低集数：{value}",
        ...
        "display.items_page": "条目数量：{count} | 第 {page}/{pages} 页",
